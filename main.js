...
function pr_display(){
    ...
    mycart.forEach(element => {
        ...
        <tr>
            ...
            <td>${element.name}</td>
            <td>${element.price}</td>
            <td><input type="number" class="input" id="up_${element.id}" value="${element.quantity}" style="border: none;"></td>
            <td class="total">${total}</td>
            <td class="remove" id="${element.id}" >X</td>
        </tr>`
    });

    $("#table").append(table+pr_list+"</table>");
   var l = $(".total").length;
   var total = 0;
    for(let i=0;i<l;i++){
 ...
 console.log(total);
 if(total==0){
     alert("Are You Sure delete your cart");
...
function display(){
    ...
    });$("#products").append(pr_list);
}
display();

$(document).on("blur" , ".input" , function(){
    var inpid= this.id;
    var inc = inpid.split("_")[1];
    var p_quantity = this.value;
     console.log(p_quantity);
    mycart.forEach(element => {
        if(element.id == inc){
            element.quantity = p_quantity;
        }  
    });
    pr_display(); 
})
